...
(function() {
  ...
  // ── API: fetch hint from Sage backend ──
  window.sageAskHint = async function(context) {
    try {
      const res = await fetch('/api/sage-hint', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          mode:           context.mode           || 'story',
          task:           context.task           || '',
          concept:        context.concept        || '',
          goal:           context.goal           || '',
          attempts:       context.attempts       || 0,
          student_action: context.student_action || '',
      ...
      if (data.message) {
        showBubble(data.message, 5000);
        swapImg(SAGE_CONFIG.smileImg, 2500);
      }
      return data.message || null;
    } catch(e) {
      // Fallback to local hint
      showBubble(getSpeech('encourage'), 4000);
      return null;
    }
  };

  // ── API: fetch journey reflection ──
  window.sageJourneyReflection = async function(journeyData) {
    try {
      const res = await fetch('/api/sage-journey', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
